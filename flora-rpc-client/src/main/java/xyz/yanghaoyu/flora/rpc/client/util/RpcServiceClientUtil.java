...
public abstract class RpcServiceClientUtil {
    public static ServiceReferenceAttribute buildServiceReferenceAttribute(
        ...
        String group         = StringUtil.getStringOrDefaultNonNull(rpcServiceReference.group(), config.group(), "no namespace found");
        String namespace     = StringUtil.getStringOrDefaultNonNull(rpcServiceReference.namespace(), config.namespace(), "no group found");
        String version       = StringUtil.getStringOrDefaultNonNull(rpcServiceReference.version(), config.version(), "no version found");
        ...
        return new ServiceReferenceAttribute(
                ZookeeperServiceKeyBuilder.aZookeeperServiceKey()
                        .group(group).namespace(namespace).interfaceName(interfaceName).version(version).build()
        );
    }
