...
public abstract class AbstractRpcClient
    ...
    public CompletableFuture<RpcResponseBody> send(RpcRequestConfig requestConfig, InetSocketAddress target) {
        Objects.requireNonNull(target, "fail to send rpc request. Cause: the target address is null. method -> " + requestConfig.methodName());
    ...
    private CompletableFuture<RpcResponseBody> doSend(RpcRequestConfig requestConfig, Channel channel) {
        ...
        waitingRequests.put(requestConfig.id(), promise);
        // write
        RpcMessage<RpcRequestBody> message = ServiceUtil.buildRpcRequestMessage(requestConfig);
        channel.writeAndFlush(message).addListener((ChannelFutureListener) future -> {
            if (future.isSuccess()) {
                LOGGER.info("requested service [{}]", requestConfig.serviceReferenceAttribute());
                return;
            }
            // error
